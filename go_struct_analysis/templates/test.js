  <script>
    ...
    function drawPreview() {
      ...
      eloads.forEach(ld => {
        ...
        for(let i=0; i<=numArrows; i++) {
          ...
          ctx.beginPath();
          if(ld.dir === 'Local Y') {
            points.push({x: pos, y: sign * arrowLen});
            ctx.moveTo(pos, sign * arrowLen); ctx.lineTo(pos, 0);
            if (arrowLen > 3) {
              ctx.lineTo(pos - 3, sign * 3); ctx.moveTo(pos, 0); ctx.lineTo(pos + 3, sign * 3);
            }
          } else {
            let gx = Math.cos(angle)*pos;
            let gy = Math.sin(angle)*pos;
            points.push({x: gx, y: gy + sign * arrowLen});
            ctx.moveTo(gx, gy + sign * arrowLen); ctx.lineTo(gx, gy);
            if (arrowLen > 3) {
              ctx.lineTo(gx - 3, gy + sign * 3); ctx.moveTo(gx, gy); ctx.lineTo(gx + 3, gy + sign * 3);
            }
          }
          ctx.stroke();
        }
        ...
        if(ld.dir === 'Local Y') {
          if (w1 === w2) {
            let sign = w1 < 0 ? 1 : -1;
            ctx.fillText(w1 + " kg/m", (len*scale)/2, sign > 0 ? -15 - 5 : 15 + 10);
          } else {
            let sign1 = w1 < 0 ? 1 : -1;
            let sign2 = w2 < 0 ? 1 : -1;
            let h1 = Math.abs(w1/maxW)*15;
            let h2 = Math.abs(w2/maxW)*15;
            if (w1 !== 0) ctx.fillText(w1 + " kg/m", 0, sign1 > 0 ? -h1 - 5 : h1 + 10);
            if (w2 !== 0) ctx.fillText(w2 + " kg/m", len*scale, sign2 > 0 ? -h2 - 5 : h2 + 10);
          }
        }
        ctx.restore();
      });
      ...
      nloads.forEach(ld => {
        ...
        ctx.strokeStyle = '#8e44ad'; ctx.fillStyle = '#8e44ad'; ctx.lineWidth = 2;
        let arrowLen = 20;
        
        if(Math.abs(ld.fx) > 0) {
          let sign = ld.fx > 0 ? 1 : -1;
          ctx.beginPath(); ctx.moveTo(-sign*arrowLen, 0); ctx.lineTo(0, 0);
          ctx.lineTo(-sign*5, 4); ctx.moveTo(0,0); ctx.lineTo(-sign*5, -4); ctx.stroke();
          ctx.save(); ctx.scale(1, -1); ctx.fillText(ld.fx + " kg", -sign*arrowLen - 10, -5); ctx.restore();
        }
        if(Math.abs(ld.fy) > 0) {
          let sign = ld.fy > 0 ? 1 : -1;
          ctx.beginPath(); ctx.moveTo(0, -sign*arrowLen); ctx.lineTo(0, 0);
          ctx.lineTo(-4, -sign*5); ctx.moveTo(0,0); ctx.lineTo(4, -sign*5); ctx.stroke();
          ctx.save(); ctx.scale(1, -1); ctx.fillText(ld.fy + " kg", 15, sign*arrowLen + 5); ctx.restore();
        }
        if(Math.abs(ld.mz) > 0) {
          ctx.beginPath(); ctx.arc(0, 0, 12, 0, Math.PI); ctx.stroke();
          let sign = ld.mz > 0 ? 1 : -1;
          ctx.beginPath(); ctx.moveTo(12, 0); ctx.lineTo(12 - sign*4, sign*5); ctx.stroke();
          ctx.save(); ctx.scale(1, -1); ctx.fillText(ld.mz + " kg.m", 15, -15); ctx.restore();
        }
        ctx.restore();
      });
    ...
    function drawDiagrams() {
        ...
        if(comboName !== 'Envelope' && currentResult.combos && currentResult.combos[comboName]) {
        ...
        let showReactions = document.getElementById('chk-reactions').checked;
        if(showReactions && targetRes && targetRes.ok) {
          targetRes.nodes.forEach(n_res => {
            let n = nodes.find(nd => nd.id === n_res.id);
            if(!n || n.support === 'Free') return;
        ...
        if (showCombEq) {
            ...
            if (cinfo) {
                ...
                for (let k in cinfo.factors) {
                    ...
                    if (f !== 0) {
                        parts.push((f > 0 && parts.length > 0 ? "+" : "") + f + k);
                    }
                }
    ...
    function generateReportWithOpts() {
      ...
      let selected = [];
      document.querySelectorAll('#report-combos-list input:checked').forEach(inp => {
          if (inp.value === 'Envelope') {
              selected.push({ name: 'Envelope', label: 'Envelope' });
          } else {
              selected.push({ name: inp.value, label: inp.dataset.label || inp.value });
          }
      });
      
      let data = { model: collectData(), result: currentResult, report_combos: selected };
    ...
    function generateTemplate() {
      ...
      if(type === 'portal') {
        ...
        addElementRow(1, 3, 1); // Left Col
        addElementRow(2, 4, 1); // Right Col
        addElementRow(3, 5, 2); // Left Gable
        addElementRow(5, 4, 2); // Right Gable
      }
    }
