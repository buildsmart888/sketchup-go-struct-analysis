module GOStructAnalysis
  module DrawGoframe
    ...
    def draw(data)
      ...
      result = data['result']
      analysis_combo = data['analysis_combo'] || 'Envelope'
      ...
      begin
        ...
        main_group.set_attribute('GOStructAnalysis', 'module', 'Frame')
        main_group.set_attribute('GOStructAnalysis', 'model_data', model_data.to_json)
        attach_goframe_analysis_attributes(main_group, model_data, result, analysis_combo)
        ...
        elements.each do |el|
          ...
          attach_goframe_element_attributes(line, model_data, result, el, n1, n2)
          ...
          if vec.valid?
            ...
            attach_goframe_element_attributes(sub_grp, model_data, result, el, n1, n2)
          end
    ...
    def attach_goframe_element_attributes(entity, model_data, result, element, node_i, node_j)
      dict = 'GOStructElement'
      result ||= {}
      result_element = (result['elements'] || []).find { |item| item['id'].to_s == element['id'].to_s } || {}
      n1_forces = result_element['n1_forces'] || {}
      n2_forces = result_element['n2_forces'] || {}
      length_m = Math.sqrt((node_j['x'].to_f - node_i['x'].to_f)**2 + (node_j['y'].to_f - node_i['y'].to_f)**2)
      max_abs_axial = [n1_forces['axial'].to_f.abs, n2_forces['axial'].to_f.abs].max
      max_abs_shear = [n1_forces['shear'].to_f.abs, n2_forces['shear'].to_f.abs].max
      max_abs_moment = [n1_forces['moment'].to_f.abs, n2_forces['moment'].to_f.abs].max
      result_nodes = result['nodes'] || []
      rni = result_nodes.find { |node| node['id'].to_s == node_i['id'].to_s } || {}
      rnj = result_nodes.find { |node| node['id'].to_s == node_j['id'].to_s } || {}
      max_node_displacement_mm = [rni, rnj].map do |node|
        dx = node['dx'].to_f
        dy = node['dy'].to_f
        Math.sqrt((dx * dx) + (dy * dy)) * 1000.0
      end.max || 0.0

      entity.set_attribute(dict, 'parent_module', 'Frame')
      entity.set_attribute(dict, 'element_kind', 'member')
      entity.set_attribute(dict, 'element_id', element['id'])
      entity.set_attribute(dict, 'node_i', element['n1'])
      entity.set_attribute(dict, 'node_j', element['n2'])
      entity.set_attribute(dict, 'section_id', element['sec'])
      entity.set_attribute(dict, 'release', element['release'].to_s)
      entity.set_attribute(dict, 'length_m', length_m)
      entity.set_attribute(dict, 'n1_axial_kg', n1_forces['axial'].to_f)
      entity.set_attribute(dict, 'n1_shear_kg', n1_forces['shear'].to_f)
      entity.set_attribute(dict, 'n1_moment_kgm', n1_forces['moment'].to_f)
      entity.set_attribute(dict, 'n2_axial_kg', n2_forces['axial'].to_f)
      entity.set_attribute(dict, 'n2_shear_kg', n2_forces['shear'].to_f)
      entity.set_attribute(dict, 'n2_moment_kgm', n2_forces['moment'].to_f)
      entity.set_attribute(dict, 'max_abs_axial_kg', max_abs_axial)
      entity.set_attribute(dict, 'max_abs_shear_kg', max_abs_shear)
      entity.set_attribute(dict, 'max_abs_moment_kgm', max_abs_moment)
      entity.set_attribute(dict, 'n1_dx_mm', rni['dx'].to_f * 1000.0)
      entity.set_attribute(dict, 'n1_dy_mm', rni['dy'].to_f * 1000.0)
      entity.set_attribute(dict, 'n2_dx_mm', rnj['dx'].to_f * 1000.0)
      entity.set_attribute(dict, 'n2_dy_mm', rnj['dy'].to_f * 1000.0)
      ...
      entity.set_attribute(dict, 'result_json', result_element.to_json)
    end

    def attach_goframe_analysis_attributes(group, model_data, result, analysis_combo = 'Envelope')
      dict = 'GOStructAnalysis'
      result ||= {}
      nodes = result['nodes'] || []
      elements = result['elements'] || []

      max_dx_m = nodes.map { |node| node['dx'].to_f.abs }.max || 0.0
      max_dy_m = nodes.map { |node| node['dy'].to_f.abs }.max || 0.0
      max_abs_displacement_m = nodes.map do |node|
        dx = node['dx'].to_f
        dy = node['dy'].to_f
        Math.sqrt((dx * dx) + (dy * dy))
      end.max || 0.0
      max_abs_reaction_fx = nodes.map { |node| node['fx'].to_f.abs }.max || 0.0
      max_abs_reaction_fy = nodes.map { |node| node['fy'].to_f.abs }.max || 0.0
      max_abs_reaction_mz = nodes.map { |node| node['mz'].to_f.abs }.max || 0.0

      max_abs_axial = 0.0
      max_abs_shear = 0.0
      max_abs_moment = 0.0
      elements.each do |element|
        n1_forces = element['n1_forces'] || {}
        n2_forces = element['n2_forces'] || {}
        max_abs_axial = [max_abs_axial, n1_forces['axial'].to_f.abs, n2_forces['axial'].to_f.abs].max
        max_abs_shear = [max_abs_shear, n1_forces['shear'].to_f.abs, n2_forces['shear'].to_f.abs].max
        max_abs_moment = [max_abs_moment, n1_forces['moment'].to_f.abs, n2_forces['moment'].to_f.abs].max
      end
      ...
      group.set_attribute(dict, 'analysis_engine', 'GOFrame')
      group.set_attribute(dict, 'analysis_combo', analysis_combo.to_s)
      ...
      group.set_attribute(dict, 'node_count', (model_data['nodes'] || []).length)
      group.set_attribute(dict, 'element_count', (model_data['elements'] || []).length)
      group.set_attribute(dict, 'max_abs_axial_kg', max_abs_axial)
      group.set_attribute(dict, 'max_abs_shear_kg', max_abs_shear)
      group.set_attribute(dict, 'max_abs_moment_kgm', max_abs_moment)
      group.set_attribute(dict, 'max_dx_m', max_dx_m)
      group.set_attribute(dict, 'max_dy_m', max_dy_m)
      group.set_attribute(dict, 'max_dx_mm', max_dx_m * 1000.0)
      group.set_attribute(dict, 'max_dy_mm', max_dy_m * 1000.0)
      group.set_attribute(dict, 'max_abs_displacement_m', max_abs_displacement_m)
      group.set_attribute(dict, 'max_abs_displacement_mm', max_abs_displacement_m * 1000.0)
      group.set_attribute(dict, 'max_abs_reaction_fx_kg', max_abs_reaction_fx)
      group.set_attribute(dict, 'max_abs_reaction_fy_kg', max_abs_reaction_fy)
      group.set_attribute(dict, 'max_abs_reaction_mz_kgm', max_abs_reaction_mz)
      group.set_attribute(dict, 'summary_json', JSON.generate({
        'frameName' => model_data['frame_name'] || model_data['name'],
        'nodeCount' => (model_data['nodes'] || []).length,
        'elementCount' => (model_data['elements'] || []).length,
        'maxAbsAxialKg' => max_abs_axial,
