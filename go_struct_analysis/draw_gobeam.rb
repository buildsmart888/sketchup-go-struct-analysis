module GOStructAnalysis
  module DrawGobeam
    ...
    def gobeam_draw3d(model_data, result)
      ...
      main_group.set_attribute('GOStructAnalysis', 'module', 'Beam')
      main_group.set_attribute('GOStructAnalysis', 'model_data', model_data.to_json)
      attach_gobeam_analysis_attributes(main_group, model_data, result)
      ...
      max_h_m = total_len_m * 0.12
      max_h_v = total_len_m * 0.08
      max_h_d = total_len_m * 0.05
      ...
      v_scale = (max_h_v / max_v)
      m_scale = (max_h_m / max_m)
      d_scale = (max_h_d / max_d)
      ...
      spans.each_with_index do |span, idx|
        ...
        attach_gobeam_span_attributes(span_grp, model_data, span, span_result, idx)
      ...
      draw_diagram = lambda do |name, layer, data_raw, scale, value_key, fmt_str, label_layer, plane=:xz|
        grp = ents.add_group
        grp.name = name
        grp.layer = layer
        ...
        data.each_cons(2) do |p1, p2|
          ...
          pts = []
          pts << Geom::Point3d.new(x1, 0, 0)

          if plane == :xz
            pts << Geom::Point3d.new(x1, 0, y1 * scale.m) if y1.abs > 1e-5
            pts << Geom::Point3d.new(x2, 0, y2 * scale.m) if y2.abs > 1e-5
          else
            pts << Geom::Point3d.new(x1, y1 * scale.m, 0) if y1.abs > 1e-5
            pts << Geom::Point3d.new(x2, y2 * scale.m, 0) if y2.abs > 1e-5
          end

          pts << Geom::Point3d.new(x2, 0, 0)
          ...
          if pts.length >= 3
            begin
              ...
              puts "GOBeam Draw Error: #{e.message}"
            end
          end
        end
        ...
        spans.each_with_index do |s, idx|
          ...
          if span_pts.length > 0
            ...
            if max_pt && (max_pt[value_key] || 0).to_f > eps
              pt_y = plane == :xy ? max_pt[value_key].to_f * scale.m : 0
              pt_z = plane == :xz ? max_pt[value_key].to_f * scale.m : 0
              pt3d = Geom::Point3d.new(max_pt['x'].to_f.m, pt_y, pt_z)

              vec = plane == :xz ? Geom::Vector3d.new(0, 0, 0.3.m) : Geom::Vector3d.new(0, 0.3.m, 0)
              txt = grp.entities.add_text("MAX " + format(fmt_str, max_pt[value_key].to_f), pt3d, vec)
              txt.layer = label_layer
            end

            if min_pt && (min_pt[value_key] || 0).to_f < -eps
              pt_y = plane == :xy ? min_pt[value_key].to_f * scale.m : 0
              pt_z = plane == :xz ? min_pt[value_key].to_f * scale.m : 0
              pt3d = Geom::Point3d.new(min_pt['x'].to_f.m, pt_y, pt_z)

              vec = plane == :xz ? Geom::Vector3d.new(0, 0, -0.3.m) : Geom::Vector3d.new(0, -0.3.m, 0)
              txt = grp.entities.add_text("MIN " + format(fmt_str, min_pt[value_key].to_f), pt3d, vec)
              txt.layer = label_layer
            end
          end
        end
      ...
      draw_diagram.call("Shear Diagram", lyr_shear, result['allSpanDiagrams']['shear'], v_scale, 'v', "%.0f kg", lyr_shear_labels, :xz)
      draw_diagram.call("Moment Diagram", lyr_moment, result['allSpanDiagrams']['moment'], m_scale, 'm', "%.0f kg-m", lyr_moment_labels, :xz)
      draw_diagram.call("Deflection Diagram", lyr_defl, result['allSpanDiagrams']['deflection'], d_scale, 'dMm', "%.2f mm", lyr_defl_labels, :xz)
    ...
    def attach_gobeam_span_attributes(entity, model_data, span, span_result, index)
      dict = 'GOStructElement'
      extremes = span_result['extremes'] || {}
      reactions = span_result['reactions'] || {}
      shear_points = span_result['shear'] || []
      moment_points = span_result['moment'] || []
      deflection_points = span_result['deflection'] || []
      max_abs_shear = shear_points.map { |point| point['v'].to_f.abs }.max || 0.0
      max_abs_moment = moment_points.map { |point| point['m'].to_f.abs }.max || 0.0
      max_abs_deflection = deflection_points.map { |point| point['dMm'].to_f.abs }.max || 0.0

      entity.set_attribute(dict, 'parent_module', 'Beam')
      entity.set_attribute(dict, 'element_kind', 'span')
      entity.set_attribute(dict, 'element_id', index + 1)
      entity.set_attribute(dict, 'span_index', index)
      entity.set_attribute(dict, 'section_shape', span['sectionShape'].to_s)
      entity.set_attribute(dict, 'section_b_m', span['sectionBM'].to_f)
      entity.set_attribute(dict, 'section_h_m', span['sectionHM'].to_f)
      entity.set_attribute(dict, 'length_m', span['lengthM'].to_f)
      entity.set_attribute(dict, 'max_shear_kg', extremes['maxShearKg'].to_f)
      entity.set_attribute(dict, 'min_shear_kg', extremes['minShearKg'].to_f)
      entity.set_attribute(dict, 'max_abs_shear_kg', max_abs_shear)
      entity.set_attribute(dict, 'max_moment_kgm', extremes['maxMomentKgM'].to_f)
      entity.set_attribute(dict, 'min_moment_kgm', extremes['minMomentKgM'].to_f)
      entity.set_attribute(dict, 'max_abs_moment_kgm', max_abs_moment)
      entity.set_attribute(dict, 'max_deflection_mm', extremes['maxDeflectionMm'].to_f)
      entity.set_attribute(dict, 'min_deflection_mm', extremes['minDeflectionMm'].to_f)
      entity.set_attribute(dict, 'max_abs_deflection_mm', max_abs_deflection)
      entity.set_attribute(dict, 'left_reaction_kg', reactions['leftKg'].to_f)
      entity.set_attribute(dict, 'right_reaction_kg', reactions['rightKg'].to_f)
      entity.set_attribute(dict, 'parent_model_hash', GOStructAnalysis.bim_model_hash(model_data))
      entity.set_attribute(dict, 'result_json', span_result.to_json)
    end

    def attach_gobeam_analysis_attributes(group, model_data, result)
      dict = 'GOStructAnalysis'
      summary = result['summary'] || {}
      all_diagrams = result['allSpanDiagrams'] || {}
      shear_points = all_diagrams['shear'] || []
      moment_points = all_diagrams['moment'] || []
      deflection_points = all_diagrams['deflection'] || []
      ...
      group.set_attribute(dict, 'analysis_ok', result['ok'] ? true : false)
      ...
      group.set_attribute(dict, 'analysis_engine', 'GOBeam')
      group.set_attribute(dict, 'analysis_combo', result['activeCombination'].to_s)
      ...
      group.set_attribute(dict, 'active_combination', result['activeCombination'].to_s)
      group.set_attribute(dict, 'span_count', summary['spanCount'].to_i)
      group.set_attribute(dict, 'total_length_m', summary['totalLengthM'].to_f)
      group.set_attribute(dict, 'max_positive_moment_kgm', summary['maxPositiveMomentKgM'].to_f)
      group.set_attribute(dict, 'max_negative_moment_kgm', summary['maxNegativeMomentKgM'].to_f)
      group.set_attribute(dict, 'max_abs_moment_kgm', max_abs_moment)
      group.set_attribute(dict, 'max_abs_shear_kg', max_abs_shear)
      group.set_attribute(dict, 'max_abs_deflection_mm', [summary['maxAbsDeflectionMm'].to_f, max_abs_deflection].max)
      group.set_attribute(dict, 'summary_json', JSON.generate({
        'beamName' => model_data['beamName'],
        'activeCombination' => result['activeCombination'],
        'spanCount' => summary['spanCount'],
        'totalLengthM' => summary['totalLengthM'],
        'maxAbsShearKg' => max_abs_shear,
        'maxAbsMomentKgM' => max_abs_moment,
        'maxAbsDeflectionMm' => [summary['maxAbsDeflectionMm'].to_f, max_abs_deflection].max
      }))
    end
