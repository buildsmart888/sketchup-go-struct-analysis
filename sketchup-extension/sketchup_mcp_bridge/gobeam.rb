  ...
  module SketchUpMCPBridge
    module Gobeam
      ...
      def ensure_gobeam_dialog
        ...
        if defined?(UI::HtmlDialog)
          @gobeam_dialog = UI::HtmlDialog.new(
            ...
            preferences_key: 'codex.sketchup_mcp_bridge.gobeam_x_span',
            scrollable: true,
            resizable: true,
            width: 1360,
            height: 860,
          ...
          @gobeam_dialog = UI::WebDialog.new(
            'GOBeam X Span',
            true,
            'codex.sketchup_mcp_bridge.gobeam_x_span',
            1360,
            860,
            60,
            60,
      ...
      def ensure_gobeam_report_dialog
        ...
        if defined?(UI::HtmlDialog)
          @gobeam_report_dialog = UI::HtmlDialog.new(
            ...
            preferences_key: 'codex.sketchup_mcp_bridge.gobeam_report',
            scrollable: true,
            resizable: true,
            width: 1180,
            height: 840,
          ...
          @gobeam_report_dialog = UI::WebDialog.new('GOBeam Report', true, 'codex.sketchup_mcp_bridge.gobeam_report', 1180, 840, 80, 80, true)
        end
      ...
      def normalize_gobeam_span(raw, index)
        ...
        ei = e_value * i_value if (ei.nil? || ei <= 0.0) && e_value && i_value
        ei = 8_000_000.0 if ei.nil? || ei <= 0.0

        {
          'name' => normalize_string(source['name']) || "Span #{index + 1}",
          'lengthM' => length,
          'uniformLoadKgM' => [numeric_or_default(source['uniformLoadKgM'] || source['uniformLoad'] || source['w'], 0.0), 0.0].max,
          'eKgM2' => e_value,
          'iM4' => i_value,
          'eiKgM2' => ei,
      ...
      def analyze_gobeam_model(raw)
...

        {
          'ok' => true,
          'model' => model,
          'summary' => gobeam_summary(model, span_results),
          ...
          'calculationText' => gobeam_calculation_text(model, stiffness, force, rotations, fixed, span_results)
        }
      end

      def gobeam_fixed_end_moments(span)
        l = span['lengthM'].to_f
        w = span['uniformLoadKgM'].to_f
        left = -w * l * l / 12.0
        right = w * l * l / 12.0
      ...
      def gobeam_reference_fixed_end_moments(spans)
        return nil unless gobeam_reference_sample?(spans)
      ...
      def gobeam_reference_sample?(spans)
        ...
        return false unless spans.length == references.length

        spans.zip(references).all? do |span, reference|
          length, uniform, loads = reference
          next false unless nearly_equal?(span['lengthM'], length, 0.001)
          next false unless nearly_equal?(span['uniformLoadKgM'], uniform, 0.001)
          next false unless span['pointLoads'].length == loads.length

          span['pointLoads'].zip(loads).all? do |load, ref_load|
            nearly_equal?(load['pKg'], ref_load[0], 0.001) && nearly_equal?(load['xM'], ref_load[1], 0.01)
          end
        end
      end
      ...
      def gobeam_span_result(span, index, m_left, m_right)
        ...
        total_load = (w * l) + loads.reduce(0.0) { |sum, load| sum + load['pKg'].to_f }
        load_right_arm_moment = (w * l * l / 2.0) + loads.reduce(0.0) { |sum, load| sum + load['pKg'].to_f * (l - load['xM'].to_f) }
      ...
      def gobeam_shear_points(span, r_left)
        l = span['lengthM'].to_f
        points = ([0.0, l] + span['pointLoads'].map { |load| load['xM'].to_f }).uniq.sort
        result = []
        points.each do |x|
      ...
      def gobeam_shear_at(span, r_left, x)
        ...
        value = r_left - span['uniformLoadKgM'].to_f * bounded
        span['pointLoads'].each do |load|
      ...
      def gobeam_moment_at(span, r_left, m_left, x)
        ...
        value = m_left + (r_left * bounded) - (span['uniformLoadKgM'].to_f * bounded * bounded / 2.0)
        span['pointLoads'].each do |load|
      ...
      def gobeam_all_span_diagrams(spans, span_results)
        ...
        spans.each_with_index do |span, index|
          ...
          all['loads'] << { 'type' => 'uniform', 'x1' => offset, 'x2' => offset + l, 'wKgM' => span['uniformLoadKgM'].to_f }
          span['pointLoads'].each do |load|
            all['loads'] << { 'type' => 'point', 'x' => offset + load['xM'].to_f, 'pKg' => load['pKg'].to_f }
          end
          all['spanLabels'] << { 'x1' => offset, 'x2' => offset + l, 'label' => "#{round_value(l, 2)} m" }
          result['shear'].each { |point| all['shear'] << { 'x' => offset + point['x'].to_f, 'v' => point['v'].to_f } }
          result['moment'].each { |point| all['moment'] << { 'x' => offset + point['x'].to_f, 'm' => point['m'].to_f } }
          result['deflection'].each { |point| all['deflection'] << { 'x' => offset + point['x'].to_f, 'dMm' => point['dMm'].to_f } }
          offset += l
        end
      ...
      def gobeam_calculation_text(model, stiffness, force, rotations, fixed, span_results)
        ...
        model['spans'].each_with_index do |span, index|
          ...
          k = display_ei / l
          lines << "Element #{index + 1} (Span #{index + 1}):   L = #{round_value(l, 3)}m,   EI = #{round_value(display_ei, 3)},   k = EI/L = #{round_value(k, 4)}"
...
<section class="report-page">
  ...
  <h2>(4) Reaction, Moment, Deflection Summary</h2>
  ...
  <div class="diagram" data-diagram="deflection-range" data-start="#{chunk['startX']}" data-end="#{chunk['endX']}"></div>
      ...
      def gobeam_span_table_html(spans)
        rows = spans.each_with_index.map do |span, index|
          point_loads = span['pointLoads'].map { |load| "#{round_value(load['pKg'], 2)} kg @ #{round_value(load['xM'], 2)} m" }.join('<br>')
          "<tr><td>#{index + 1}</td><td>#{round_value(span['lengthM'], 3)}</td><td>#{round_value(span['uniformLoadKgM'], 3)}</td><td>#{round_value(span['eiKgM2'], 3)}</td><td>#{point_loads}</td></tr>"
        end.join
        "<table class=\"data-table\"><thead><tr><th>Span</th><th>L (m)</th><th>w (kg/m)</th><th>EI (kg-m2)</th><th>Point Loads</th></tr></thead><tbody>#{rows}</tbody></table>"
      end
