...
class DiagramCanvas(QWidget):
    ...
    def _display_value(self, quantity: str, value: float) -> float:
        if quantity in {"n_kg", "v_kg"}:
            return self._units.force(value)
        if quantity == "m_kg_m":
            return self._units.moment(value)
        return self._units.length(value / 1000.0)

    def _value_label(self, quantity: str, value: float) -> str:
        return self._display_number_label(quantity, self._display_value(quantity, value))

    def _display_number_label(self, quantity: str, value: float) -> str:
        if quantity == "v_mm":
            return self._units.format_displacement(value / self._units.length_factor)
        return f"{value:,.3f}"
    ...
    def mouseMoveEvent(self, event) -> None:  # type: ignore[no-untyped-def]
        ...
        label, _, _ = QUANTITIES[self._quantity]
        QToolTip.showText(
            event.globalPosition().toPoint(),
            f"{label}: {self._value_label(self._quantity, float(point[self._quantity]))} {self._unit_label(self._quantity)}\n"
            f"x: {self._units.length(float(point['x_m'])):,.3f} {self._units.length_unit}",
            self,
        )
    ...
    def paintEvent(self, event) -> None:  # type: ignore[no-untyped-def]
        ...
        if not points:
            return
        label, _, color = QUANTITIES[self._quantity]
        unit = self._unit_label(self._quantity)
        values = [self._display_value(self._quantity, float(point[self._quantity])) for point in points]
        ...
        length = self._units.length(float(self._member["length_m"]))
        ...
        painter.drawText(QPointF(left + chart_width - 68.0, self.height() - 12.0), f"L = {length:.3f} {self._units.length_unit}")
        painter.drawText(QPointF(7.0, top + 9.0), self._display_number_label(self._quantity, upper))
        painter.drawText(QPointF(7.0, top + chart_height), self._display_number_label(self._quantity, lower))

        line = QPolygonF([screen(self._units.length(float(point["x_m"])), self._display_value(self._quantity, float(point[self._quantity]))) for point in points])
        ...
        for key, prefix in (("max", "max"), ("min", "min")):
            ...
            if not item:
                continue
            point = screen(self._units.length(float(item["x_m"])), self._display_value(self._quantity, float(item["value"])))
            painter.setPen(QPen(color, 1.5))
            painter.setBrush(QColor("#ffffff"))
            painter.drawEllipse(point, 3.4, 3.4)
            painter.setPen(QColor("#334155"))
            governing = f" [{item['combo']}]" if item.get("combo") else ""
            painter.drawText(point + QPointF(5.0, -6.0), f"{prefix} {self._value_label(self._quantity, float(item['value']))}{governing}")
...
class FrameDiagramsPanel(QWidget):
    ...
    def set_unit_system(self, key: str) -> None:
        self.canvas.set_unit_system(key)
