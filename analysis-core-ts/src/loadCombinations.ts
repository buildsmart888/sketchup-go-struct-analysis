import type { LoadCombination, NodalLoad2D, StructuralModel } from "./types.js";

export function activeCombination(model: StructuralModel): LoadCombination {
  const found = model.loadCombinations.find((item) => item.name === model.activeCombination);
  if (found) return found;
  if (model.loadCombinations[0]) return model.loadCombinations[0];
  return { name: "1.0DL", method: "ASD", factors: { DL: 1 } };
}

export function combinedNodalLoads(model: StructuralModel): Map<string, { fxKg: number; fyKg: number; mzKgM: number }> {
  const combination = activeCombination(model);
  const loads = new Map<string, { fxKg: number; fyKg: number; mzKgM: number }>();
