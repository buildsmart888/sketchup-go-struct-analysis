...
export function analyzeTruss2D(model: StructuralModel): Truss2DResult {
  ...
  for (const member of model.members) {
    if (member.module && member.module !== "truss_2d") continue;
    ...
    if (length <= 0) throw new Error(`Zero-length truss member ${member.id}`);
    const c = dx / length;
    const s = dy / length;
    ...
    for (let r = 0; r < 4; r += 1) {
      for (let col = 0; col < 4; col += 1) stiffness[map[r]][map[col]] += k * local[r][col];
    }
  }
...
function restrainedDofs(model: StructuralModel, nodeIndex: Map<string, number>, dofs: Array<"ux" | "uy" | "rz">): Set<number> {
  const restrained = new Set<number>();
  for (const support of model.supports) {
    const index = nodeIndex.get(support.nodeId);
    if (index === undefined) continue;
    dofs.forEach((dof, offset) => {
      if (support.restraints[dof]) restrained.add(index * dofs.length + offset);
    });
  }
  return restrained;
}

function trussMemberForces(model: StructuralModel, displacementVector: number[], nodeIndex: Map<string, number>): TrussMemberForce[] {
  const materialById = new Map(model.materials.map((material) => [material.id, material]));
  const sectionById = new Map(model.sections.map((section) => [section.id, section]));
