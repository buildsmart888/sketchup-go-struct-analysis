...
export function analyzeFrame2D(model) {
    ...
    for (const member of model.members) {
        if (member.module && member.module !== "frame_2d")
            continue;
        ...
        for (let r = 0; r < 6; r += 1) {
            for (let col = 0; col < 6; col += 1)
                stiffness[map[r]][map[col]] += global[r][col];
        }
    }
    ...
    for (const support of model.supports) {
        ...
        if (index === undefined)
            continue;
        if (support.restraints.ux)
            restrained.add(index * 3);
        if (support.restraints.uy)
            restrained.add(index * 3 + 1);
        if (support.restraints.rz)
            restrained.add(index * 3 + 2);
    }
...
function frameTransform(x1, y1, x2, y2) {
    const dx = x2 - x1;
    const dy = y2 - y1;
    const length = Math.hypot(dx, dy);
    if (length <= 0)
        throw new Error("Zero-length frame member");
    const c = dx / length;
    const s = dy / length;
    return {
        length,
...
function frameMemberEndForces(model, displacementVector, nodeIndex) {
    const materialById = new Map(model.materials.map((material) => [material.id, material]));
    const sectionById = new Map(model.sections.map((section) => [section.id, section]));
    return model.members.filter((member) => !member.module || member.module === "frame_2d").map((member) => {
        ...
        const localEndForces = multiplyVector(local, localDisplacement);
